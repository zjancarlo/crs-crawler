...
try:
    courseCatalog = pickle.load(open("courseCatalog.p", "rb"))
except Exception as e:
    courseCatalog = {}
...
def page_process():
    for letter in string.ascii_lowercase:
        if not os.path.exists("Course Catalog/"):
            os.makedirs("Course Catalog/")
        fileName = "Course Catalog/catalog_{}.txt".format(letter)
...
def catalog_crawl():
    ...
    for letter in string.ascii_lowercase:
        ...
        downloadPage(url=indexURL, fileName=fileName, filePath=filePath)
...
def sched_process():
    for letter in string.ascii_uppercase:
        if not os.path.exists("Class Schedules/"):
            os.makedirs("Class Schedules/")
        ...
        while True and (i < len(searchLines)):
            ...
            if searchLines[i].find(endBody) != -1:
                ...
                #classMatch = re.search(".*>(?P<class_name>[\w\s-]*)<.*", className)
                ...
                print "Added class %s" % classMatch.group("class_name") # (classMatch.group("class_name"))
                startFound = True
            elif startFound:
                if searchLines[i].find(blockEnd) != -1:
                    break
...
def sched_crawl():
    baseURL = "https://crs.upd.edu.ph/schedule/120172/"
    for letter in string.ascii_uppercase:
        indexURL = baseURL + letter
